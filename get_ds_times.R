...
get_ds_times <- function(dataset = "fb-final", mult = 1) {
    ...
    x <- map(files, \(f) {
        ...
        list(Scen = Scen, Rep = Rep,
             Samples = mult * res$params$nsample,
             Seconds = ceiling(mult * elapsed))
    }) |>
        rbindlist()
    ...
    message(str_glue("Runs took about {a} (longest {b})",
                     a = seconds_to_period(ceiling(mult * mean(run_times))),
                     b = seconds_to_period(ceiling(mult * max(run_times)))))
    invisible(x)
}
