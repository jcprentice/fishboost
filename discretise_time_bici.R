discretise_time_bici <- function(popn, params) {
    ...
    map(seq_len(nrow(popn2)), \(i) {
        ...
        # popn2[id == 1, .(id, Tinf, Tinc, Tsign, Tdeath, parasites, initial_comp, comp_status)]
        ...
        if (is.na(event_times[["Tdeath"]])) {
            ...
            # If no Tdeath *and* no Tsign, check for parasites
            if (is.na(event_times[["Tsign"]] && has_parasites == FALSE)) {
                event_times[["Tsign"]] <- tr_max + time_step
            }
        }
