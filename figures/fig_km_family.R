...
fig_km_family <- function(dataset = "fb-test",
    ...
    # Set families: half-sib families, and families in different trials, are
    ...
    # Donor families are any families with a donor
    ...
    data_sv[, gp := .GRP, .(id, family, trial)]
    ...
    data_svm <- tmp |>
        ...
        .(id, family, trial, donor, src, es, gp, variable)]

    data_svm[, `:=`(survival = nafill(survival, type = "locf"),
                    str = str_c(src, es, fifelse(donor == 1, "d", "r"), sep = "_") |>
                        str_remove("X_"))] |>
    ...
    data_sim <- data_svm[src == "sim",
                         ...
                         .(trial, donor, src, es, str, variable, time)] |>
    ...
    scm <- rowwiseDT(
        ...
        "fb_hi_d",  "Data Seeder highest",        "#33A02C",
        "sim_hi_d", "Simulation Seeder highest",  "#B2DF8A",
        "fb_lo_d",  "Data Seeder lowest",         "#E31A1C",
        "sim_lo_d", "Simulation Seeder lowest",   "#FB9A99",
        "fb_hi_r",  "Data Contact highest",       "#1F78B4",
        "sim_hi_r", "Simulation Contact highest", "#A6CEE3",
        "fb_lo_r",  "Data Contact lowest",        "#FF7F00",
        "sim_lo_r", "Simulation Contact lowest",  "#FDBF6F"
    )
...

{
    ...
    p_leg2 <- data.table(x = rep(seq_len(100), 2),
                         y = replicate(2, cumsum(rnorm(100))) |> c(),
                         z = rep(scm2$breaks, each = 100)) |>
        ggplot() +
        aes(x, y, group = z, colour = z) +
        geom_line() +
    ...
    plt_fb1 <- plot_grid(p_fb1a + theme(legend.position = "none"),
                         p_fb1b + theme(legend.position = "none"),
                         labels = "auto",
                         label_size = 10,
                         align = "hv", nrow = 1)

    plt_fbl1 <- plot_grid(plt_fb1, p_legend2,
                          ncol = 1, rel_heights = c(1, 0.08))
    plt_fbl1

    ggsave("figures/km_fb1.pdf", plt_fbl1,
           width = 18.3, height = 9, units = "cm", dpi = "print")
    ggsave("figures/km_fb1.png", plt_fbl1,
           width = 18.3, height = 9, units = "cm", dpi = "print")
    ...
    plt_fb12 <- plot_grid(p_fb1a + theme(legend.position = "none"),
                          p_fb1b + theme(legend.position = "none"),
                          p_fb2a + theme(legend.position = "none"),
                          p_fb2b + theme(legend.position = "none"),
                          labels = "auto",
                          label_size = 10,
                          align = "hv", nrow = 2)

    plt_fbl12 <- plot_grid(plt_fb12, p_legend2,
                           ncol = 1, rel_heights = c(1, 0.08))
    plt_fbl12

    ggsave("gfx/km_fb12.pdf", plt_fbl12,
           width = 18.3, height = 9, units = "cm", dpi = "print")
    ggsave("gfx/km_fb12.png", plt_fbl12,
           width = 18.3, height = 9, units = "cm", dpi = "print")
}
