...
Next we take the `params` file and use it to generate a population with groups, traits, weights, and fixed effects applied to the phenotypes. Note that you these are piped into each other, but you can save the intermediate result at any point to examine what's happening (e.g. for bug fixing).

First `make_pedigree()` creates a population with the appropriate structure (either cp
...
If we look at some of the individuals, we can see that they now have individual values for their genotypes, and different phenotypes

```{r}
popn2[sdp == "progeny", map(.SD, round, 2),
     .SDcols = str_subset(names(popn2), "sus|inf|tol")][1:5]
```
...
There's one more step here, we need to specify the FE values we want to use. It's just a matrix containing all the combinations.
