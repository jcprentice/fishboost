...
data_set <- glue("sim-G_Da-{x}{y}",
                 x = if (convergence) 1 else 2,
                 y = if (sire22) "-mpi" else "")

# Variable parameters ----
protocol <- rbind(
    data.table(description = "Sim FB Trial 1, SEIDR, Gs",
               setup = "fb1",
               label = "s1a",
               use_traits = "s",
               donor_fe = "none",
    ...
    data.table(description = "Sim FB Trial 1, SEIDR, Gsi",
               setup = "fb1",
               label = "s1b",
               use_traits = "si",
               donor_fe = "none",
    ...
    data.table(description = "Sim FB Trial 1, SEIDR, Gs, donor LIDR",
               setup = "fb1",
               label = "s1c",
               use_traits = "s",
               donor_fe = "lidr",
    ...
    data.table(description = "Sim FB Trial 1, SEIDR, Gsi, donor LIDR",
               setup = "fb1",
               label = "s1d",
               use_traits = "si",
               donor_fe = "lidr",
...
missing_cols <- c("model_type", "data_set", "name", "use_traits", "vars", "covars",
                  ...
                  "sire_version", "nsample_per_gen", "quench", "quench_power",
