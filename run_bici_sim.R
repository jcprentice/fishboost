...
run_bici_sim <- function(dataset = "fb-test",
    ...
    # Open config file and get no. of reps (reps_in_cf)
    ...
    reps_in_cf <- str_split_i(parts[[part_n]], "=", 2) |> as.numeric()

    if (nreps %notin% c(NA, 0, reps_in_cf)) {
    ...
    flatten_bici_states(list(dataset = dataset,
                             name = name,
                             bici_cmd = bici_cmd))
}
