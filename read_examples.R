...
dt <- read_xml("../sire21/examples/example16.xml") %>%
    ...
    read.table(text=.) %>%
    as.data.table()
setnames(dt, c("id", "group", "IC", "Tinf", "Tinc", "Tsym", "Trec", "inf", "sus", "rec"))

dt[, sdp := fifelse(is.na(group), "sire", "progeny")]
setcolorder(dt, c("id", "sdp"))
dt[, id := as.integer(gsub("Ind", "", id)) + 1]
...
dt[, Trec := as.numeric(fifelse(Trec == "no", "0", Trec, NA))]
...
dt2 <- merge.data.table(dt[id %in% sires, .(id, sus, inf, rec)],
                        estimated_BVs[id %in% sires, .(id, est_sus = g_a, est_inf = f_a, est_rec = r_a)],
                        by = "id")

ranks <- dt2[, lapply(.SD, rank)]
# ranks[, `:=`(rec = seq(.N), est_rec = rank(sort(runif(.N)) + rnorm(.N, 0, 0.2)))]
...
cor.test(ranks$rec, ranks$est_rec, method = "spearman")
