discretise_time_sire <- function(popn, params) {
    {
    ...
    possible_states <- function(t, event_times) {
        ...
        str_flatten(compartments[fwd <= t & t < bwd], "|")
    }
    
    clip <- function(x, lo, hi) x[lo <= x & x <= hi]
    
    final_comp <- compartments[[length(compartments)]]
    
    comp_status <- map_chr(seq_len(nrow(popn)), \(i) {
        if (popn$sdp[i] != "progeny") return("NA")
        
        # popn[id == i, .(id, Tinf, Tinc, Tsym, Tdeath, parasites, initial_comp, comp_status)]
        
        tr_max <- tmax[[str_c("t", popn$trial[[i]])]]
        init_comp <- compartments[[popn$donor[[i]] + 1L]]
        
        if (init_comp == final_comp) {
            return(str_glue("[{final_comp}:0],[{final_comp}:{tr_max}]"))
        }
        
        is_donor <- init_comp == compartments[[2L]]
        has_parasites <- popn$parasites[[i]]
        event_times <- popn[i, ..timings] |> unlist() |> ceil_ts()
        ...
        states[[1]] <- init_comp
        ns <- length(states)
        if (str_starts(states[[ns]], "S") && has_parasites) {
        ...
        str_c("[", states[idxs], ":", times1[idxs], "]", collapse = ",")
    })
