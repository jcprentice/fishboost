...
get_RPs <- function() {
    ...
    x <- km_data$km_data[[1]]$data[src == "sim", .(trial, donor, Tsign, Tdeath)]
    ...
    # Cut out values where both Tsign and Tdeath2 are NA (need at least 1 value)
    x <- x[!is.na(Tsign) | !is.na(Tdeath)]

    # Trial 1
    f1 <- x[trial == 1 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 1 seeders
    f1s <- x[trial == 1 & donor == 1 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 1 contact
    f1c <- x[trial == 1 & donor == 0 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 2 seeders
    f2s <- x[trial == 2 & donor == 1 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 2 contact
    f2c <- x[trial == 2 & donor == 0 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
...
get_RPs_fb <- function(fix_seeders = c(10, 80)) {
    ...
    x <- fb_data[sdp == "progeny", .(trial, donor, Tsign, Tdeath, parasites)]

    x[Tdeath == c(104, 160)[trial], Tdeath := NA]
    if (any(fix_seeders > 0)) {
        message("Reclassifying seeders")
        x[donor == 1 & Tsign > fix_seeders[trial], donor := 0]
    }
    ...
    x[, `:=`(left = fcase(!is.na(Tsign) & !is.na(Tdeath), pmax(Tdeath - Tsign - 1, 0),
                          !is.na(Tsign) & is.na(Tdeath), c(104, 160)[trial] - Tsign,
                          is.na(Tsign) & !is.na(Tdeath), 0,
                          default = NA),
             right = fcase(!is.na(Tsign) & !is.na(Tdeath), pmax(Tdeath - Tsign + 1, 0),
                           !is.na(Tsign) & is.na(Tdeath), NA,
                           is.na(Tsign) & !is.na(Tdeath), Tdeath,
                           default = NA))]

    # Trial 1
    tmp <- x[trial == 1 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 1 seeders
    tmp <- x[trial == 1 & donor == 1 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 1 contact
    tmp <- x[trial == 1 & donor == 0 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 2 seeders
    tmp <- x[trial == 2 & donor == 1 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
    ...
    # Trial 2 contact
    tmp <- x[trial == 2 & donor == 0 & !is.na(Tsign) &
                 !is.na(Tdeath) & Tdeath > Tsign, Tdeath - Tsign] |>
        fitdist("gamma")
...
get_DPs <- function() {
    km_data <- readRDS("fb-final-old/results/km.rds")
    x <- km_data$km_data[[1]]$data[src == "sim", .(trial, donor, Tsign)]
    ...
    # Cut out values where Tsign is NA
    x <- x[!is.na(Tsign)]

    # Trial 1
    f1 <- x[trial == 1 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f1$estimate[["mean"]] = f1$estimate[["shape"]] / f1$estimate[["rate"]]
    message("Trial 2 contact:")
    print(round(f1$estimate[vals], 2))
    ...
    # Trial 1 seeders
    f1s <- x[trial == 1 & donor == 1 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f1s$estimate[["mean"]] = f1s$estimate[["shape"]] / f1s$estimate[["rate"]]
    message("Trial 1 seeders:")
    print(round(f1s$estimate[vals], 2))
    ...
    # Trial 1 contact
    f1c <- x[trial == 1 & donor == 0 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f1c$estimate[["mean"]] = f1c$estimate[["shape"]] / f1c$estimate[["rate"]]
    message("Trial 1 contact:")
    print(round(f1c$estimate[vals], 2))
    ...
    # Trial 2
    f2 <- x[trial == 2 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f2$estimate[["mean"]] = f2$estimate[["shape"]] / f2$estimate[["rate"]]
    message("Trial 2 contact:")
    print(round(f2$estimate[vals], 2))
    ...
    # Trial 2 seeders
    f2s <- x[trial == 2 & donor == 1 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f2s$estimate[["mean"]] = f2s$estimate[["shape"]] / f2s$estimate[["rate"]]
    message("Trial 2 seeders:")
    print(round(f2s$estimate[vals], 2))
    ...
    # Trial 2 contact
    f2c <- x[trial == 2 & donor == 0 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f2c$estimate[["mean"]] = f2c$estimate[["shape"]] / f2c$estimate[["rate"]]
    message("Trial 2 contact:")
    print(round(f2c$estimate[vals], 2))
...
get_DPs_fb <- function(fix_seeders = c(10, 80)) {
    # Process data
    fb_data <- readRDS("fb_data/fb_12.rds")
    x <- fb_data[sdp == "progeny", .(trial, donor, Tsign, Tdeath)]
    ...
    x[, `:=`(left = fcase(!is.na(Tsign), pmax(Tsign - 1, 0),
                          is.na(Tsign) & !is.na(Tdeath), 0,
                          default = NA),
             right = fcase(!is.na(Tsign), Tsign,
                           is.na(Tsign) & !is.na(Tdeath), Tdeath,
                           default = NA))]

    # Trial 1
    tmp <- x[trial == 1 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f1 <- fitdistcens(x[trial == 1, .(left, right)], "gamma",
                       start = list(shape = tmp$estimate[["shape"]],
                                    rate = tmp$estimate[["rate"]]))
    ...
    # Trial 1 seeders
    tmp <- x[trial == 1 & donor == 1 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f1s <- fitdistcens(x[trial == 1 & donor == 1, .(left, right)], "gamma",
                       start = list(shape = tmp$estimate[["shape"]],
                                    rate = tmp$estimate[["rate"]]))
    ...
    # Trial 1 contact
    tmp <- x[trial == 1 & donor == 0 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f1c <- fitdistcens(x[trial == 1 & donor == 0, .(left, right)], "gamma",
                       start = list(shape = tmp$estimate[["shape"]],
                                    rate = tmp$estimate[["rate"]]))
    ...
    # Trial 2
    tmp <- x[trial == 2 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f2 <- fitdistcens(x[trial == 2, .(left, right)], "gamma",
                       start = list(shape = tmp$estimate[["shape"]],
                                    rate = tmp$estimate[["rate"]]))
    ...
    # Trial 2 seeders
    tmp <- x[trial == 2 & donor == 1 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f2s <- fitdistcens(x[trial == 2 & donor == 1, .(left, right)], "gamma",
                       start = list(shape = tmp$estimate[["shape"]],
                                    rate = tmp$estimate[["rate"]]))
    ...
    # Trial 2 contact
    tmp <- x[trial == 2 & donor == 0 & !is.na(Tsign), Tsign] |>
        fitdist("gamma")
    f2c <- fitdistcens(x[trial == 2 & donor == 0, .(left, right)], "gamma",
                       start = list(shape = tmp$estimate[["shape"]],
                                    rate = tmp$estimate[["rate"]]))
