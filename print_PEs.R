...
print_PEs <- function(dataset = "fb-test", scen = 2) {
    ...
    p2[, `:=`(
        Mean   = sprintf(str_glue("%5 .{d}f", d = d), mean),
        Median = sprintf(str_glue("%5 .{d}f", d = d), median)
    )]

    p2[, hdi95 := sprintf(str_glue("(%5 .{d}f, %5 .{d}f)", d = d),
                          hdi95min, hdi95max), .I]

    p2[, .(Parameter, Prior, Mean, Median, hdi95)]
}
