...
next_sis_ni_event <- function(pop, epi_time) {
    as.list(pop[, .(.I,
...
# Main model ----
model_SIS <- function(traits, params) {
    ...
    epi_time <- 0.0
    while (get_sis_infectives(pop) > 0L && epi_time < tmax) {
        ...
        ni_event <- next_sis_ni_event(pop, epi_time)
        ...
        if (epi_time + dt < t_next_event) {
            ...
            if (status == "I") {
                ...
                set(pop, id_next_event, "status", "S")
            } else {
