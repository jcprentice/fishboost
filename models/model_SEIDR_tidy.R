...
next_seidr_ni_event <- function(X, epi_time) {
    X %>% 
        mutate(I = dplyr::row_number(),
               Tinc2   = if_else(Tinc > epi_time, Tinc, Inf),
               Tsym2   = if_else(Tsym > epi_time, Tsym, Inf),
               Tdeath2 = if_else(Tdeath > epi_time, Tdeath, Inf),
               Tmin = pmin(Tinc2, Tsym2, Tdeath2)) %>%
        slice_min(Tmin) %>%
        pull(I)
}
...
model_SEIDR <- function(popn, params) {
    ...
    Y <- map(Xs, \(X) {
        ...
        while (get_seidr_infectives(X) > 0L) {
            if (DEBUG) message("time = ", t = signif(epi_time, 5))
            ...
            X <- X %>%
                mutate(group_inf = r_beta * GE * mean(if_else(status %in% c("I", "D"), inf, 0.0)),
                       event_rate = if_else(status == "S", sus * group_inf, 0.0))
            
            if (DEBUG) message("Group infection is ", X$group_inf[[1]] |> signif(3))
            ...
            if (DEBUG) message(str_glue("Next NI event id = {id} at t = {t}",
                                    id = id_next_event,
                                    t = signif(t_next_event, 5)))
            
            # generate random timestep ----
            total_event_rate <- sum(X$event_rate)
            
            # calculate dt if infections event rate > 0
            dt <- if (total_event_rate > 0.0) rexp(1L, total_event_rate) else Inf
            
            
            if (DEBUG) message(str_glue("Total infections event rate = {tev}",
                                    tev = signif(total_event_rate, 5)))
            ...
            if (epi_time + dt < t_next_event) {
                ...
                infectives <- X |>
                    mutate(I = tidytable::row_number()) |>
                    tidytable::select(I, status, inf) |>
                    filter(status %in% c("I", "D"))
                ...
                if (DEBUG) {
                    message("group infectivities = \n",
                            str_flatten(capture.output(X[, group_inf[1], group]), "\n"))
                }
                ...
                status <- X$status[[id_next_event]] |> as.character()
                
                if (status == "E") {
                    if (DEBUG) message("ID ", id_next_event, ": E -> I")
                    X[id_next_event, "status"] <- "I"
    ...
    final_t <- Y[, max(Tdeath, na.rm = TRUE)] |> signif(5)
