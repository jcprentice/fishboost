...
generate_km_data_sire <- function(dataset = "fb-final",
    ...
    popns <- map(fb_data, \(fb) {
        ...
        if ("ntrials" %notin% params) {
            params$ntrials <- if (str_detect(params$setup, "_12")) 2L else 1L
        }
        if ("cov_G" %notin% names(params)) {
            params$cov_G <- params$cov_E <- params$Sigma_G
        }
        params$sim_new_data  <- "r"
